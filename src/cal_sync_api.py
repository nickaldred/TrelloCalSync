...
def add_event(event: Event):
    ...
    if not calendar_handler or not db_handler:
        return Response(
            status_code=400, content="Calendar or database handler not found"
        )
    ...
    if event_id:
        ...
        if db_result:
            ...
            return Response(
                status_code=400,
                content="Event not added, unable to add to database",
            )
    else:
        print("Unable to add calendar event to calendar")
        return Response(
            status_code=400,
            content="Event not added, unable to add to calendar",
        )

    return Response(status_code=201, content="Event added")


@app.get("/get_event/{event_id}")
def get_event(trello_card_id: str) -> Response:
    ...
    return Response(status_code=200, content=calendar_event)


@app.delete("/delete_event/{event_id}")
def delete_event(trello_card_id: str) -> Response:
    ...
    calendar_event: dict = calendar_handler.delete_event_by_id(
        event_data["event_id"], event_data["calendar_id"]
    )
